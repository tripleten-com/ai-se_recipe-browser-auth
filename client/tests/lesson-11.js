...
test('api.ts exports logoutUser', () => {
  assert(has(api, 'logoutUser'), 'Export a logoutUser function that calls POST /auth/logout');
});

test('api.ts does not send an Authorization header', () => {
  assert(!has(api, 'Authorization'), 'Remove the Authorization header from the request helper');
});

test('api.ts does not read from localStorage', () => {
  assert(!has(api, 'localStorage'), 'Remove the localStorage.getItem call from the request helper');
});
...
test('Behavior tests pass', () => {
  ...
  assert(result.ok, 'Run `npm test -- tests/lib/lesson-11.behavior.test.tsx` in client/ for details');
});
