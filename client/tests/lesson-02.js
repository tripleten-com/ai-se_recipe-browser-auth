...
test('Uses createContext', () => {
  assert(has(authCtx, 'createContext'), 'Import and call createContext from "react"');
});

test('Exports AuthProvider', () => {
  assert(has(authCtx, 'AuthProvider'), 'Create and export a function named AuthProvider');
});

test('Exports useAuth hook', () => {
  assert(has(authCtx, 'useAuth'), 'Export a useAuth convenience hook that returns useContext(AuthContext)');
});

test('Has isAuthenticated state', () => {
  assert(has(authCtx, 'isAuthenticated'), 'Add isAuthenticated state to AuthProvider with initial value false');
});

test('Has currentUser state', () => {
  assert(has(authCtx, 'currentUser'), 'Add currentUser state to AuthProvider with initial value null');
});
...
test('main.tsx wraps App with AuthProvider', () => {
  assert(has(main, 'AuthProvider'), 'Import AuthProvider and wrap <App /> with it in client/src/main.tsx');
});
