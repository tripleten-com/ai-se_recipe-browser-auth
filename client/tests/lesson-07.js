...
test('api.ts exports getCurrentUser', () => {
  assert(has(api, 'getCurrentUser'), 'Add an exported getCurrentUser() function to api.ts');
});

test('getCurrentUser calls GET /users/me', () => {
  assert(has(api, '/users/me'), 'getCurrentUser should call GET ${BASE_URL}/users/me using the request helper');
});

test('AuthContext has isLoading state', () => {
  assert(has(authCtx, 'isLoading'), 'Add isLoading: boolean to AuthContextValue and useState(true) in AuthProvider');
});

test('AuthContext uses useEffect', () => {
  assert(has(authCtx, 'useEffect'), 'Import useEffect and add a token-check effect that runs on mount');
});

test('AuthContext calls getCurrentUser in the effect', () => {
  assert(has(authCtx, 'getCurrentUser'), 'Import getCurrentUser from api.ts and call it inside the useEffect');
});

test('ProtectedRoute handles isLoading', () => {
  assert(has(protect, 'isLoading'), 'Read isLoading from useAuth() and return null while it is true in both ProtectedRoute and PublicRoute');
});
