...
test("loginUser return type includes token and CurrentUser", () => {
  assert(
    has(api, "Promise<{ token: string; user: CurrentUser }>"),
    "Annotate loginUser's return type as Promise<{ token: string; user: CurrentUser }>",
  );
});

test("registerUser return type includes CurrentUser", () => {
  assert(
    has(api, "Promise<{ user: CurrentUser }>"),
    "Annotate registerUser's return type as Promise<{ user: CurrentUser }>",
  );
});
