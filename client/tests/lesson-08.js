...
test('App.tsx calls toggleLike from api.ts', () => {
  assert(has(app, 'toggleLike'), 'Import toggleLike from api.ts and call it in a handleToggleLike function');
});

test('App.tsx passes onToggleLike to child components', () => {
  assert(has(app, 'onToggleLike'), 'Pass onToggleLike as a prop to HomePage and FavoritesPage');
});
...
test('RecipeCard accepts onToggleLike prop', () => {
  assert(has(card, 'onToggleLike'), 'Add onToggleLike: (id: string) => void to RecipeCard props and call it on the heart button');
});

test('RecipeCard reads recipe.likes', () => {
  assert(has(card, 'likes'), 'Derive isLiked from recipe.likes.includes(currentUser?.userId)');
});
...
test('FavoritesPage filters recipes by likes', () => {
  assert(has(favorites, 'filter'), 'Filter the recipes array to show only recipes whose likes array includes the current user ID');
});

test('FavoritesPage uses likes', () => {
  assert(has(favorites, 'likes'), 'Check recipe.likes.includes(currentUser.userId) to identify liked recipes');
});
